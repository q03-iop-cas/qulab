...
me_power = 3             # dBm
ex_freq  = 7.637         # GHz
ex21_freq = 7.3052       # GHz
ex21_att  = 0            # dB
ex_power = -30           # dBm
ex21_power = 18          # dBm
heterodyne_freq = 50e6   # Hz
READ_OUT = 20000         # ns
ME_LEN   = 1000          # ns
PI_LEN   = 100           # ns
TRIGGER_DELAY = 180      # ns
pc_range = np.linspace(-5, 18, 47)                    # dBm
...
class MyApp(Application):
    __title__   = 'ATS var fp & pc (dispersive measurement)'
    __version__ = 'v0.1'
    
    last_t = -1
    
    def set_sweep(self):
        self.sweep_channel("pc", pc_range,
                           long_name='Coupler Power', unit='dBm')
    ...
    def set_datas(self):
        ...
        self.data.template(
            name="ATS_fp_pc", fname='ATS_fp_pc_{autoname}.txt',
            index=[Index('Probe Frequency', unit='GHz'), Index('Coupler Power', unit='dBm')],
            cols=[Column('P0', with_err=True, unit='mV'),
                  Column('P1', with_err=True, unit='mV'),
                  Column('P2', with_err=True, unit='mV')],
            doc = doc)
            
        self.data.targets("ATS_fp_pc")
        self.data_manager.save_raw = False
            
    def set_plots(self):
        def get_data(app = self):
            x    = app.data["ATS_fp_pc"]['Probe Frequency']
            y    = app.data["ATS_fp_pc"]['Coupler Power']
            z, e = app.data["ATS_fp_pc"]['P0']
            return x, y, z
    ...
    @step()
    def set_freq(self, fp, pc):
        self.cfg['ex_mw'].write(":FREQ:CW %.13e" % (fp*1e9))
        self.cfg['ex21'].write(":POWER %.8e" % (pc+ex21_att))
        time.sleep(0.01)
    ...
    def measurement(self):
        ...
        self.data["ATS_fp_pc"].append(P0 = (P0, e0),
                                   P1 = (P1, e1),
                                   P2 = (P2, e2))
