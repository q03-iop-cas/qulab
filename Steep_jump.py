...
class MyApp(Application):
    ...
    def set_datas(self):
        ...
        self.data.template(
            name="SteepJumpS0", fname='sjs0_{autoname}.txt',
            index=[Index('Power', unit='dBm')],
            cols=[Column('Amplitude', with_err=True, unit='mV')],
            doc = doc)
            
        self.data.targets("SteepJumpS0")
            
    def set_plots(self):
        
        def get_data(app = self):
            x    = app.data["SteepJumpS0"]['Power']
            y, e = app.data["SteepJumpS0"]['Amplitude']
            return x, y
            
        self.plot.add_item(curve("Steep Jump",
                                 "Power (dBm)",
    ...
    def measurement(self):
        ...
        e  = 1e3*np.abs(A).std()
        self.data["SteepJumpS0"].append(Amplitude = (P1, e))
