...
READOUT_LEN = 20
n = 5000
vRange = np.arange(0.01, 4.5, 0.01)
...
for DCV in [0.67, 0.68, 0.69, 0.7, 0.71, 0.72, 0.73, 0.74]:
    V, flux = bais_wav.set_Phi(DCV)
...
'''.format(
    ...
    Lab.savetxt('MRT0_%f_'%flux, np.array([x,y]).T, header=header)
    logger.info('MRT0 Finished')

    plt.plot(x, y, label=('Flux=%f'%flux))
plt.show()
